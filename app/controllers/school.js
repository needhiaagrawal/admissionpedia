import {
    getSchoolService,
    getSchoolDetailService,
    getSchoolSearchFiltersService,
    schoolShortlistedService,
    shortlistedSchool,
  ...
  export const getSchools = async (req, res) => {
    try {
      ...
      const dbResult = await getSchoolService(
        keyword,
        board,
        gender,
        district,
  ...
  export const findSchoolByRegistrationNumber = async (req, res) => {
    try {
      console.log("Reqistration Number-----------", req.query);
      const board = req.query.board || null;
      const registrationNumber = req.query.registrationNumber || null;
  
      const limit = STANDARD_SEARCH_LIMIT;
      const dbResult = await getSchoolByRegistrationNumber(
        board,
        registrationNumber,
        limit
      );
      ...
      logger.error("Error in req findSchool" + err.toString());
      res.status(httpStatus.INTERNAL_SERVER_ERROR).send("Something went wrong");
    }
  };
  ...
  export const getSearchFilter = async (req, res) => {
    try {
      ...
      logger.info(
        "Get School Search Result successful" + JSON.stringify(dbResult)
      );
      res.send(dbResult);
    } catch (err) {
      logger.error("Error in req getSchools" + err.toString());
  ...
  export const schoolShortlisted = async (req, res) => {
    try {
      ...
      const dbResult = await schoolShortlistedService(
        token,
        schoolId,
        notification
      );
  ...
  export const selfSignupSchool = async (req, res) => {
    try {
